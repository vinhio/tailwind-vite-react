...
const HeaderComponent: React.FC = () => {
  ...
  const toggleMode = () => {
    if (localStorage.getItem('theme')) {
      switchMode(localStorage.getItem('theme') === 'light');
    } else {
      switchMode(!document.documentElement.classList.contains('dark'));
    }
  };
  ...
  return (
    <Fragment>
      ...
      <header className="container mx-auto mt-10 px-6 text-center h-40 md:h-20">
        {/* Logo */}
        <div
          className="bg-logo-light-mode dark:bg-logo-dark-mode bg-no-repeat h-20 w-48 mx-auto md:mx-0 md:absolute top-10 left-10"></div>
        {/* Menu */}
        <div
          className="flex items-center justify-center space-x-4 md:space-x-10 md:absolute top-12 right-10">
          <a href="#features" className="hover:text-accent-cyan">Features</a>
          <a href="#testimonials" className="hover:text-accent-cyan">Testimonials</a>
          ...
          <button onClick={toggleMode}
            ...
            {/* Dark SVG Icon */}
            <svg className={"w-5 h-5 " + (dark ? "hidden" : "")}
